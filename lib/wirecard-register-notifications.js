...
module.exports = (appSdk) => {
  const setupWirecard = () => {
    ...
    const registerWirecardProcedures = (row) => {
      return new Promise((resolve, reject) => {
        ...
        rq(options, (erro, resp, result) => {
          if (erro || resp.statusCode >= 400) {
            logger.error('Wirecard register procedures error:', resp.body)
          } else {
            resolve(row)
          }
        })
      ...
      return new Promise((resolve, reject) => {
        ...
        rq.get(options, async (err, resp, body) => {
          ...
          try {
            ...
            return appSdk.apiRequest(row.store_id, resource, method, update, auth)
              .then(() => resolve(row))
          } catch (error) {
            throw error
          }
    ...
    const updateDB = (row) => {
      return new Promise((resolve, reject) => {
        ...
        db.run(query, [1, row.store_id], erro => {
          if (erro) {
            logger.error('Setup wirecard failed:', erro)
          } else {
            resolve()
          }
        })
    ...
    db.each(query, [0], (err, row) => {
      setTimeout(async () => {
        if (!err) {
          if (row) {
            registerWirecardProcedures(row)
              .then(savePublicKey)
              .then(updateDB)
              .then(() => {
                logger.log('Application setted up procedures and public key')
                logger.log('Application setted up procedures and public key')
              })
          }
        }
  ...
  setupWirecard()
  setInterval(setupWirecard, 3000)
}
