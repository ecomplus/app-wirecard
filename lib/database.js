...
module.exports = {
  ...
  transactions: {
    save: (transactionId, status, storeId) => {
      const sql = 'INSERT INTO wirecard_transactions (transaction_id, current_status, store_id) VALUES(?,?,?)'
      return dbRunPromise(sql, [transactionId, status, storeId])
    },
    get: (transactionId) => {
      return new Promise((resolve, reject) => {
  ...
  getStores: () => new Promise((resolve, reject) => {
    const query = 'SELECT DISTINCT store_id FROM ecomplus_app_auth ORDER BY store_id'
