  ...
  return {
    ...
    amount: {
      ...
      subtotals: {
        ...
        discount: paymentDiscount(payload)
      }
    },
      ...
      if (paymentCredit) {
        ...
        if (installmentTax && installmentNoTax) {
          if (payload.params.installments_number > installmentNoTax.number) {
            return Math.round((payload.params.amount.total * installmentTax.tax_value) * 100)
          }
        }
      }
    }
...
const paymentDiscount = payload => {
  if (payload.params.payment_method.code === 'banking_billet') {
    if (payload.application.hasOwnProperty('hidden_data') && payload.application.hidden_data.hasOwnProperty('payment_options')) {
      let bankingBillet = payload.application.hidden_data.payment_options.find(payment => payment.type === 'banking_billet')
      if (bankingBillet) {
        if (bankingBillet.hasOwnProperty('discount')) {
          switch (bankingBillet.discount.type) {
            case 'percentage':
              return Math.round((payload.params.amount.total * bankingBillet.discount.value) * 100)
            case 'fixed':
              return Math.round((payload.params.amount.total - bankingBillet.discount.value) * 100)
            default:break
          }
