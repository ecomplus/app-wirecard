...
const requestOrder = (payload) => {
  return {
    ...
    amount: {
      ...
      subtotals: {
        shipping: Math.abs(payload.params.amount.freight * 100),
        addition: paymentAddition(payload),
        discount: Math.abs(payload.params.amount.discount * 100) || 0
      }
    },
      ...
      if (paymentCredit) {
        ...
        if (installmentTax && installmentNoTax) {
          if (payload.params.installments_number > installmentNoTax.number) {
            return Math.abs((payload.params.amount.total * installmentTax.tax_value) * 100)
          }
        }
      }
    }
...
let orderItems = (items) => {
  ...
  products = items.map(item => {
    return {
      ...
      price: Math.abs(item.price * 100) // wirecard só aceita inteiro
    }
  })
  return products
}
