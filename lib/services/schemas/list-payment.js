...
const listPaymentSchema = {
  discount: (payment, application) => {
    if (payment.discount) {
      let discount = application.hidden_data.payment_options.filter(el => {
...
const listPaymentsOptions = application => {
  ...
  //
  discount = application.hidden_data.payment_options.find(hidden => (hidden.type === 'banking_billet' || hidden.type === 'online_debit') && (hidden.discount.hasOwnProperty('default') && hidden.discount.default === true))
