...
const createOrder = (order) => {
  console.log(JSON.stringify(schemas.requestOrder(order)))
  return new Promise((resolve, reject) => {
    let options = {
    ...
    rq.post(options, (erro, resp, body) => {
      if (resp.statusCode >= 400) {
        console.log('Erro com a requisição ao wirecard\n:' + JSON.stringify(schemas.requestOrder(order)))
        reject(new Error('Erro com a requisição ao wirecard\n:' + JSON.stringify(resp.body)))
      }
      resolve(body)
    })
  })
...
let payWith = {
  ...
  card: (order) => {
    return {
      installmentCount: order.params.installments_number,
      statementDescriptor: order.application.hidden_data.statement_descriptor.substr(0, 12) || 'Wirecard',
      fundingInstrument: {
        method: 'CREDIT_CARD',
        creditCard: {
