...
const updateAppToken = () => {
  let query = 'SELECT authentication_id, store_id FROM ecomplus_app WHERE updated_at < datetime("now", "-8 hours")'
  let i = 0
  sql.each(query, (err, row) => {
    ...
    setTimeout(async () => {
      if (!err) {
        try {
          getAppAuthentication(row.store_id, row.authentication_id)
        } catch (error) {
    ...
    setTimeout(async () => {
      if (!err) {
        try {
          ...
          rq.post(options, async (erro, resp, body) => {
            if (resp.statusCode >= 400) {
              logger.error('Erro com a solicitação ao Wir                returnecard.' + resp.body)
              return
            }
            body = JSON.parse(body)
            let params = {
...
let setAppAuthentication = async (request, response) => {
  let app = await sql.select({ store_id: request.body.store_id }, 'ecomplus_app_auth').catch(e => console.log(e))
  if (!app) {
    let params = {
    ...
    await sql.insert(params, 'ecomplus_app_auth').catch(e => {
    ...
    let params = {
      ...
      store_id: request.body.store_id ? request.body.store_id : app.store_id
    }
    await sql.update(params, { store_id: request.body.store_id }, 'ecomplus_app_auth').catch(e => {
