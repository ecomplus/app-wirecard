...
class WebHooks {
    ...
    if (!notification) {
      ...
      // e atualizo na API da ecomplus
        ...
        if (update) {
          ...
          return this.patchEcomplus(xstoreid, orderNumber, wirecardPayment).catch(e => console.log(e))
        }
      }
    }
  }
  ...
  async patchEcomplus (xstoreid, number, payment) {
    return new Promise(async (resolve, reject) => {
      let orders = await this.getOrdersEcom(xstoreid, payment.id).catch(e => console.log(e))
      if (orders) {
        let orderId
        let transactionId
          ...
          if (element.transactions) {
            ...
            element.transactions.forEach(trans => {
              if (trans.intermediator.transaction_code === payment.id) {
                transactionId = trans.intermediator._id
              }
            })
          }
        })
        let options = {
          ...
          body: {
            ...
            date_time: '2018-12-01T10:00:00.612Z', // payment.updatedAt,
