...
module.exports = {
  ...
  create: (request, response) => {
    ...
    orders.order.new(request.body).then(retorno => {
      paymentRequest(request.body, retorno).then(retorno => {
        response.status(200)
  ...
  boleto: (order) => {
    return {
      ...
      fundingInstrument: {
        method: 'BOLETO',
        boleto: {
          expirationDate: moment(new Date()).add(order.hidden_data.payment_options.expiration_days, 'days').toISOString().slice(0, 10),
          instructionLines: {
            first: 'Atenção',
      ...
      fundingInstrument: {
        ...
        creditCard: {
          ...
          holder: {
            ...
            birthdate: order.params.buyers.birth_date.year + '-' + order.params.buyers.birth_date.month + '-' + order.params.buyers.birth_date.day,
            taxDocument: {
              type: 'CPF',
    ...
    return {
      ...
      transaction: {
        ...
        banking_billet: {
          ...
          text_lines: [
            payment.fundingInstrument.boleto.instructionLines.first || null,
            payment.fundingInstrument.boleto.instructionLines.second || null,
            payment.fundingInstrument.boleto.instructionLines.third || null
          ],
          valid_thru: payment.fundingInstrument.boleto.expirationDate.toISOString()
        },
        creditor_fees: {
        ...
        intermediator: {
          ...
          payment_link: payment._links.payBoleto.redirectHref,
          status: {
