...
module.exports = {
  ...
  create: (request, response) => {
    ...
    orders.order.new(request.body, request.headers['x-store-id']).then(retorno => {
...
let optionsPaymentSchema = (params) => {
  let freeInstallments = params.application.hidden_data.payment_options.find(hidden => {
    if (typeof hidden.installments !== 'undefined') {
      return hidden.installments.find(option => {
        if (option.tax === false) {
