...
module.exports = (payload, storeId, auth) => {
  logger.log(JSON.stringify(payload))
  const { params, application } = payload
  const resource = (application.hidden_data.hasOwnProperty('sandbox') && application.hidden_data.sandbox === true) ? 'https://sandbox.moip.com.br/v2/orders' : 'https://api.moip.com.br/v2/orders'
...
const parseToSchema = (params, application) => {
  return {
    ...
    customer: {
      ...
      shippingAddress: {
        city: params.to.city,
        district: params.to.borough ? params.to.borough.substr(0, 45) : undefined,
