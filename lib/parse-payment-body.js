  ...
  if (params.payment_method && params.payment_method.code === 'credit_card') {
    ...
    if (installmentsOptions) {
      if (params.installments_number > installmentsOptions.max_number_w_tax) {
        // tax
        let valueWithTax
        for (let i = 1; i < installmentsOptions.max_number; i++) {
          if (i === params.installments_number) {
            valueWithTax = Number(tablePrice(amount.total, i, installmentsOptions.tax_value))
          }
        }
