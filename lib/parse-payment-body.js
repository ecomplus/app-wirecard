...
module.exports = body => {
  ...
  if (params.payment_method && params.payment_method.code === 'credit_card' && params.installments_number >= 2) {
    ...
    if (installmentsOptions) {
      if ((params.installments_number > installmentsOptions.max_number_w_tax) || (installmentsOptions.interest_free_min_amount > amount.total)) {
        // tax
        const valueWithTax = Math.abs(tablePrice(amount.total, params.installments_number, installmentsOptions.tax_value))
